"""The HSR dataset: obtain, standardise, verify.
...
``label`` is the heat-shock-element call and decides which compounds the table
holds. ``cytotox`` is the viability call from the counter-screen over the same
library, and is null for a compound that screen did not call.
...
Run as a command:
    ...
    python -m vp_hsr.data fetch --verify
    python -m vp_hsr.data verify
"""
...
TABLE_PATH = DATA_DIR / "hsr_tox21.parquet"
EXAMPLE_PATH = DATA_DIR / "example" / "hsr_example.parquet"
...
def _missing(path: Path) -> str:
    return (
        ...
        "`python -m vp_hsr.data fetch` to rebuild it from PubChem."
    )
        ...
        rows.append(
            {
                ...
                "potency_um": float(np.median(reported))
                if len(reported)
                else float("nan"),
                "n_calls": len(group),
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="python -m vp_hsr.data")
