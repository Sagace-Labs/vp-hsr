...
from vp_hsr import contract as hsr_contract
from vp_hsr import data as hsr_data
from vp_hsr import model as hsr_model
from vp_hsr.target import CYTOTOX, TARGET
...
def build_version(
    ...
    *,
    ...
    X = fingerprints.featurize(smiles, hsr_model.FEATURES)

    fitted = {}
    for output in hsr_contract.column_names():
        column = OUTPUT_LABELS[output]
        rows = hsr_data.labelled(table, column)
...
def _write_version(
    ...
    *,
    ...
    record = {
        ...
        "supersedes": supersedes,
        "reason": reason,
        "signature": hsr_contract.as_manifest_table(),
        "dataset": {
            "name": "Tox21 heat shock response qHTS",
            "source": (
    ...
    print(
        ...
        f"  next: python -m vp_hsr.evaluate --version {version}",
        file=sys.stderr,
    )
    return directory
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="python -m vp_hsr.train")
